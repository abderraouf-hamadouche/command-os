...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-8 text-center">
            <h1 class="display-4 mb-3">Welcome to {{ config('app.name') }}</h1>
            <p class="text-muted mb-4">By Admins For Admins</p>
            {{-- Search Bar --}}
            <div class="mb-4 mx-auto" style="max-width: 500px;">
                <input type="text" id="search-input" class="form-control form-control-lg" placeholder="Type to search commands..." autocomplete="off">
            </div>
            {{-- Search Results Container --}}
            <div id="search-results-container" class="text-start mb-5 mx-auto" ></div>
            {{-- Section Links --}}
            <div class="d-grid gap-3 d-sm-flex justify-content-sm-center">
                <a href="{{ route('command.index') }}" class="btn btn-primary btn-lg px-4 gap-3">Commandes </a>
                <a href="{{ route('intervention.index') }}" class="btn btn-outline-secondary btn-lg px-4"> Interventions</a>
            </div>
        </div>
    </div>
</div>
...
<style>
    #search-results-container .list-group-item-action {
        ...
        margin-bottom: -1px; /* To collapse borders */
    }
    ...
    .search-result-item {
        display: flex;
        align-items: baseline; /* Aligns text based on their bottom line */
        gap: 1.5rem; /* Creates space between the name and description */
    }
    .command-name {
        flex: 0 0 180px; /* Fixed width of 180px, does not grow or shrink */
    ...
    */
    .command-description {
        flex: 1 1 auto;  /* Allows this column to grow */
        color: #6c757d;   /* Muted text color for the description */
        white-space: nowrap;
        overflow: hidden;
