...
<div class="container py-4">
                        ...
                        <li class="list-group-item etape-item">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-10">
                                    ...
                                    <input type="hidden" name="etapes[{{ $etape->id }}][next_step_ko]" class="next-step-ko-input" value="{{ $etape->next_step_ko }}">

                                        <!-- Fail next step selection -->

                                        <select class="form-select w-auto form-select-sm mt-2 ko-select"  style="{{ $etape->next_step_ko ? 'display: block;' : 'display: none;' }}">
                                            <option value="">-- Choose step --</option>
                                            @foreach($intervention->etapes as $e)
                                                <option value="{{ $e->id }}" 
                                                    {{ $etape->next_step_ko == $e->id ? 'selected' : '' }}>
                                                    Etape {{ $e->position }}
                                                </option>
                                            @endforeach
                                    </select>
                                ...
                                <div class="col-md-2 text-end btn-group" role="group">
                                    ...
                                    <button type="button" class="btn btn-sm toggle-ko {{ $etape->next_step_ko ? 'btn-warning' : 'btn-outline-warning' }}" data-id="{{ $etape->id }}">⚠️</button>
    ...
    <div class="card shadow-sm">
        <div class="card-header">
            <h3 class="h5 mb-0">Add a New Step</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('etape.store') }}" method="POST">
                ...
                <div class="row g-3">
                    {{-- Command Selection --}}
                    <div class="col-md-6">
                        <label for="commande_id" class="form-label">Command</label>
                        <select class="form-select" id="commande_id" name="commande_id" required>
                            ...
                            @foreach ($commands as $command)
                                <option value="{{ $command->id }}" data-parametres='{{ json_encode($command->parametres) }}'>{{ $command->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        <label for="parametre_id" class="form-label">Parameter (Optional)</label>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="note" name="note" placeholder="e.g., Check the status of the container">
                    </div>
                </div>
